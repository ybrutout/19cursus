...
int		main( void ) {
	...
	ints_t				deposits( d, d + d_size );//pas comprendre
	...
	Account::displayAccountsInfos();//demande l'affichage des infos du compte ici équivaut à la 9Ème ligne de return
	std::for_each( acc_begin, acc_end, std::mem_fun_ref( &Account::displayStatus ) );//display status équivaut a montrer le détail de chaque compte
	...
	Account::displayAccountsInfos();//equivaut a la ligne 26 de return
	...
	Account::displayAccountsInfos();//équivaut à la ligne 43
	std::for_each( acc_begin, acc_end, std::mem_fun_ref( &Account::displayStatus ) );
